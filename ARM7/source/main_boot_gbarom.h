...
*/
  ...
  __asm {
	...
	_readFirmware((u32)0x07FE70, 0x1, &settings1);
	_readFirmware((u32)0x07FF70, 0x1, &settings2);
	
	if (settings1 > settings2) {
		_readFirmware((u32)0x07FE00, 0x70, (u8*)0x027FFC80);
	} else {
		_readFirmware((u32)0x07FF00, 0x70, (u8*)0x027FFC80);
	}
}
	  ...
	  switch(RESET){
		...
		case RESET_HomeBrew: 
		{
		    ...
		    *((vu32*)0x027FFE34) = IPC6->RESET_BootAddress;	// Bootloader start address
		    ...
		    if(1)
		    { // Final stage: Clear ARM7 internal memory.
...
*/
		...
		case RESET_Commercial:
		{
            ...
            uint32 a9exec = *((vu32*)0x027FFE24) ;
            *((vu32*)0x027FFFC8) = a9exec ;
            // Jump to boot loader
            asm 
            {
                swi 0<<16
