...
void Splash_Init(char *ani_path)
{
  /*const char *pfn;	  
  pfn=DefaultDataPath "/skin/splash.ani" ;
  if(!FullPath_FileExistsAnsi(pfn))
	  return ;*/

  if(!FullPath_FileExistsAnsi(ani_path))
	  ani_path = DefaultDataPath "/boot/splash.ani" ;
  
  const char* pfullalias=ConvertFullPath_Ansi2Alias(ani_path);
  pfh = FAT2_fopen_AliasForRead(pfullalias);
  //pfh=Shell_FAT_fopen_Data("boot/splash.ani");
  if(pfh==NULL) return;
