...
u32 SkinDataCount;
TLangData SkinData[SkinMaxCount];

u32 MulLangDataCount;
TLangData MulLangData[MulLangDataMaxCount];
...
void Lang_Load(void)
{
  ...
  LangInitAndLoadFontExt();
  
  pf=Shell_FAT_fopen_Language_messages();
  if(pf==NULL){
...
const char* Mul_Lang_GetUTF8_internal(const char *pItemName,const char *pErrorMsg)
{  
  u32 hash=CalcItemHash(pItemName);
  
  for(u32 idx=0;idx<MulLangDataCount;idx++){
    TLangData *pld=&MulLangData[idx];
    if(hash==pld->ItemHash){
      if(isStrEqual(pItemName,pld->pItem)==true){
        return(pld->pValue);
      }
...
void SkinData_Load(void)
{

	  FAT_FILE *pf=Shell_FAT_fopen_Mul_Skin();
	  if(pf==NULL){
	    _consolePrintf("Fatal error: Not found message file.\n");
	    ShowLogHalt();
	  }
	      ...
	      if((pbuf[ofs]==0x0d)||(pbuf[ofs]==0x0a)){
	        ...
	        if((linelen!=0)&&(linebuf[0]!=';')&&(linebuf[0]!='#')){
	          ...
	          if(ofs!=0){
	            ...
	            TLangData *pld=&SkinData[SkinDataCount];
	            pld->pItem=str_AllocateCopy(&linebuf[0]);
	            pld->ItemHash=CalcItemHash(pld->pItem);
	            pld->pValue=str_AllocateCopy(&linebuf[ofs+1]);
	            SkinDataCount++;
