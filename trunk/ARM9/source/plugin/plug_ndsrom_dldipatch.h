...
data_t dldiMagicString[] = "\xDD\xA5\x8D\xBF Chishm";
...
bool DLDIPatch(data_t *appFileData,u32 appFileSize,data_t *dldiFileData,u32 dldiFileSize)
{
  ...
  DLDI_patchOffset = patchOffset;
  EFS_patchOffset = quickFind (appFileData, efsMagicString, appFileSize, sizeof(efsMagicString)/sizeof(char));
  if (patchOffset < 0) {
    _consolePrintf("Does not have a DLDI section\n");
    return(false); // return EXIT_NO_DLDI_SECTION;
  }
