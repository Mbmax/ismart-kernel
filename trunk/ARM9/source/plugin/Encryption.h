...
static int DetectRomType(){
	...
	int i=0x200;
	for (; i<0x4000; i++)
		if (FAT_fgetc(fNDS)) return ROMTYPE_MASKROM;	// found something odd ;)
	return ROMTYPE_ENCRSECURE;
}

static u32 card_hash[0x412];
...
static NOINLINE void encrypt(u32 *magic, u32 *arg1, u32 *arg2){
	u32 a,b,c;
	a = *arg1;
	b = *arg2;
	int i=0;
...
static NOINLINE void decrypt(u32 *magic, u32 *arg1, u32 *arg2){
	u32 a,b,c;
	a = *arg1;
	b = *arg2;
	int i=17;
...
static NOINLINE void init2(u32 *magic, u32 a[3]){
	encrypt(magic, a+2, a+1);
	encrypt(magic, a+1, a+0);
	update_hashtable(magic, (u8*)a);
}

static NOINLINE void init1(u32 cardheader_gamecode){
...
static  void EnDecryptSecureArea(char *ndsfilename, char endecrypt_option)
{
	...
	FAT2_fread(&header, 512, 1, fNDS);
	int romType = DetectRomType();

	FAT2_fseek(fNDS, 0x4000, SEEK_SET);
	FAT2_fread(data, 1, 0x4000, fNDS);
	...
	if (romType == ROMTYPE_NDSDUMPED)
	{
		...
		else
		{
			...
			if(encrypt_arm9(*(u32 *)header.gamecode, data)){FAT2_fclose(fNDS);return;}
...
*/
			// rounds table
			FAT2_fseek(fNDS, rounds_offsets, SEEK_SET);
			FAT2_fwrite(card_hash + 0, 4, 18, fNDS);
			// S-boxes
			for (i=0; i<4; i++)
			{
				FAT2_fseek(fNDS, sbox_offsets + 4*256*i, SEEK_SET);
				FAT2_fwrite(card_hash + 18 + i*256, 4, 256, fNDS);	// s
			}
			// test patterns
			FAT2_fseek(fNDS, 0x3000, SEEK_SET);

			for(i=0x3000; i<0x3008; i++) FAT_fputc("\xFF\x00\xFF\x00\xAA\x55\xAA\x55"[i - 0x3000], fNDS);
			for(i=0x3008; i<0x3200; i++) FAT_fputc((u8)i, fNDS);
			for(i=0x3200; i<0x3400; i++) FAT_fputc((u8)(0xFF-i), fNDS);
			for(i=0x3400; i<0x3600; i++) FAT_fputc(0x00, fNDS);
			for(i=0x3600; i<0x3800; i++) FAT_fputc(0xFF, fNDS);
			for(i=0x3800; i<0x3A00; i++) FAT_fputc(0x0F, fNDS);
			for(i=0x3A00; i<0x3C00; i++) FAT_fputc(0xF0, fNDS);
			for(i=0x3C00; i<0x3E00; i++) FAT_fputc(0x55, fNDS);
			for(i=0x3E00; i<0x4000-1; i++) FAT_fputc(0xAA, fNDS);
			FAT_fputc(0x00, fNDS);
			// write secure 0x800
			FAT2_fseek(fNDS, 0x4000, SEEK_SET);
			FAT2_fwrite(data, 1, 0x800, fNDS);
			...
			header.header_crc = CalcHeaderCRC(header);
			FAT2_fseek(fNDS, 0, SEEK_SET);
			FAT2_fwrite(&header, 512, 1, fNDS);
...
int main(int argc, char **argv){
	...
	int i=1;
	for(;i<argc;i++){
		fprintf(stderr,"Processing %s... ",argv[i]);encr_data
...
void EncryptSecureArea(char *ndsfilename){EnDecryptSecureArea(ndsfilename, 'e');}
void DecryptSecureArea(char *ndsfilename){EnDecryptSecureArea(ndsfilename, 'd');}
