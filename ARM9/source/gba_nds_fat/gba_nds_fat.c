...
///////////////////////add by cpy 2009-4-8
static  u32 fatBufferCurSector_cpy;
static char *fatBuffer_cpy;
#include "Memtool.h"
////////////////////////
...
int wrkDirSector;
int wrkDirOffset;
//Remove LFN Last Cluster
u32 lastwrkDirCluster;
...
DIR_ENT FAT_GetDirEntry ( u32 dirCluster, int entry, int origin)
{
  ...
  lastwrkDirCluster=wrkDirCluster;
  found = false;
  notFound = false;
  do {
    wrkDirOffset++;
...
int FAT_remove (const char* path)
{
  ...
  //Remove lfn Directoty entry	
  	DIR_ENT dirEntrytemp;
  	bool nextFlag=true;
  	while(nextFlag)
  	{
  		if(wrkDirOffset>0)
  		{
  			wrkDirOffset--;
  			dirEntrytemp=((DIR_ENT*)globalBuffer)[wrkDirOffset];
  			if(dirEntrytemp.attrib==ATTRIB_LFN)
  			{
  				((DIR_ENT*)globalBuffer)[wrkDirOffset].name[0] = FILE_FREE;
  				nextFlag=true;			
  			}
  			else//Over in one sector
  			{	
  				disc_SystemWriteSector ( (wrkDirCluster == FAT16_ROOT_DIR_CLUSTER ? filesysRootDir : FAT_ClustToSect(wrkDirCluster)) + wrkDirSector , globalBuffer);
  				disc_CacheFlush();
  				nextFlag=false;
  		...
  		else //Over in two sector
  		{
  			//Save preview 
  			disc_SystemWriteSector ((wrkDirCluster == FAT16_ROOT_DIR_CLUSTER ? filesysRootDir : FAT_ClustToSect(wrkDirCluster)) + wrkDirSector , globalBuffer);
  			disc_CacheFlush();
  			if(wrkDirSector>0)//last sector
  			{
  				wrkDirSector--;
  				disc_SystemReadSector ( (wrkDirCluster == FAT16_ROOT_DIR_CLUSTER ? filesysRootDir : FAT_ClustToSect(wrkDirCluster)) + wrkDirSector , globalBuffer);
  				wrkDirOffset=BYTE_PER_READ / sizeof (DIR_ENT);
  			}
  			else//last cluster
  			{
  				wrkDirCluster=lastwrkDirCluster;
  				wrkDirSector = filesysSecPerClus-1;
  				wrkDirOffset=BYTE_PER_READ / sizeof (DIR_ENT);
  				disc_SystemReadSector ( (wrkDirCluster == FAT16_ROOT_DIR_CLUSTER ? filesysRootDir : FAT_ClustToSect(wrkDirCluster)) + wrkDirSector , globalBuffer);
  			}			
  		}
...
int FAT_mkdir (const char* path)
{
  ...
  MemSet32CPU(0x00,entries,BYTE_PER_READ);
  	for(i=1;i<filesysSecPerClus;i++)
  		disc_SystemWriteSector(FAT_ClustToSect(newDirCluster)+i,entries);
  // Flush any sectors in disc cache
  disc_CacheFlush();
  return 0;
}
...
///////////////////////////////////////////////////////
//需要两个全局变量
//_VARS_IN_RAM char fatBuffer_cpy[BYTE_PER_READ]; //add by cpy
//u32 fatBufferCurSector_cpy;
//add by cpy 20090408
static inline bool FAT_WriteFatEntry_cpy (u32 cluster, u32 value)
{
	u32 sector;
	int offset;
	...
	switch (filesysType) 
	{
		...
		case FS_FAT16:
			...
			// If FAT buffer contains wrong sector
			if (sector != fatBufferCurSector_cpy)
			{
				disc_WriteSector(fatBufferCurSector_cpy, fatBuffer_cpy);
				fatBufferCurSector_cpy = sector;
				disc_ReadSector(fatBufferCurSector_cpy, fatBuffer_cpy);
			}
			
			((u16*)fatBuffer_cpy)[offset] = (value & 0xFFFF);
		...
		case FS_FAT32:
			...
			(((u32*)fatBuffer_cpy)[offset]) =  value;
...
////////////////////////////////////////////
//输入:1,patch 路径,现在暂时用不到
//		2,filesize 簇的整数陪
//输出:0,创建文件失败
//	  1,创建文件成功
int FAT_quickCreatefile(const char *path,u32 filesize) //filesize Byte
{
	u32 startCluster;
	u32 clusterCount;
	u32 curClus;
	DIR_ENT dirEntry;
	/*FAT_FILE* pfile;
	if((pfile = FAT_fopen(path, "w+"))==NULL)
	{
		_consolePrint("createfile false!!\n");
		return 0;
	}
	...
	*/
	...
	//分配一个全局的buff
    if ((fatBuffer_cpy = (char*)safemalloc(512)) == NULL) 
    { 
		_consolePrint("malloc mem false!!\n");
		return 0;
    } 
    //_consolePrint("malloc mem succe!!\n");

	//dirEntry = FAT_DirEntFromPath (path);
	//startCluster = dirEntry.startCluster | dirEntry.startClusterHigh <<16 ;
    startCluster = FAT2_GetFirstCluster();
	//_consolePrintf("startCluster = %x\n",startCluster);
	//_consolePrintf("filesysBytePerClus = %x\n",filesysBytePerClus);
	clusterCount = filesize/filesysBytePerClus -1 ;
	//_consolePrintf("clusterCount = %d\n",clusterCount);
	if(clusterCount < 0) clusterCount = 0;
	if(filesize%filesysBytePerClus > 0) clusterCount++;

	curClus = startCluster;

	// Start at first valid cluster

	fatFirstFree = CLUSTER_FIRST;  // 都是全局变量
	
	//_consolePrintf("\n0_fatFirstFree:%d",fatFirstFree);
	
	//_consolePrintf("\n*1*_fatBufferCurSector:%d\n",fatBufferCurSector);
	
	fatBufferCurSector_cpy = fatBufferCurSector;
	disc_ReadSector(fatBufferCurSector_cpy, fatBuffer_cpy);
	...
	{	
		...
		if ((curClus >= CLUSTER_FIRST) && (curClus < fatLastCluster))
		{
			// Update the linked from FAT entry
			FAT_WriteFatEntry_cpy (curClus, fatFirstFree);
		}
		curClus = fatFirstFree ;
		fatFirstFree++;		
	}
	// Create the linked to FAT entry		
	_consolePrintf("\ncurClus:%x\n",curClus);
	FAT_WriteFatEntry_cpy (curClus, CLUSTER_EOF);
	disc_WriteSector(fatBufferCurSector_cpy, fatBuffer_cpy);//上面的循环最后一次没有写进去

	safefree(fatBuffer_cpy);//
	fatBuffer_cpy = NULL;
	return 1;
}
///////////////////////////////////////////////////////////////////////cpy end
...
//add by cpy 20080807
#ifdef CAN_WRITE_TO_DISC
int FAT_flag_E5 (const char* path )  //删除原文件,不清理fat表
{
	DIR_ENT dirEntry;
	u32 oldWorkDirCluster;
	char checkFilename[13];
...
static void CreatFile( char *pFullPathAliasFrom,const char *pPathAliasTo,DIR_ENT dirEntryTmp)
{
   ...
   *(s+4)=0;
    ...
    {   
        ...
        if(fstype2 == FT_NONE)
        {
           ...
           //	FAT2_fclose(filePK);
        	if(!FAT2_fopen_CreateForWrite_ins_AddDirEntry(a_saver,w_saver,dirEntryTmp)){
        	    _consolePrintf("Fatal error: Can not add dir entry.\n");
        	    ShowLogHalt();        	    
        	  }
...
static int WriteBack(const char* path,DIR_ENT dirEntryTmp)
{
	  DIR_ENT dirEntry;
	  u32 oldWorkDirCluster;
	  char checkFilename[13];
	  ...
	  disc_SystemReadSector ( (wrkDirCluster == FAT16_ROOT_DIR_CLUSTER ? filesysRootDir : FAT_ClustToSect(wrkDirCluster)) + wrkDirSector , globalBuffer);
	 //((DIR_ENT*)globalBuffer)[wrkDirOffset].name[0] = FILE_FREE;
	  memcpy(&((DIR_ENT*)globalBuffer)[wrkDirOffset],&dirEntryTmp,sizeof(DIR_ENT));
	
}
int FAT_movefile(const char *from,const char * to,UnicodeChar *DesPath,UnicodeChar *FileName)
{
  DIR_ENT dirEntry;
  dirEntry = FAT_DirEntFromPath(from);	 
  char p[256];
  strcpy(p,from);
  CreatFile(p,to,dirEntry); 
  FAT_flag_E5 (from); 
}
