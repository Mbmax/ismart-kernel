...
void BootROM_SetInfo_NoLaunch(const UnicodeChar *pPathUnicode,const UnicodeChar *pFilenameUnicode)
{
  ...
  strcpy(pBNRN,pbody->NDSFullPathFilenameAlias);
  BootROMInfo.Execute=false;
...
void BootROM_Execute(void)
{
  ...
  if(strcmp("####",ID)==0) gl_homebrew=true;
  if(strcmp("PASS",ID)==0) gl_homebrew=true;
  if((ID[0]==0x3d)&&(ID[1]==0x84)&&(ID[2]==0x82)&&(ID[3]==0x0a)) gl_homebrew=true;
    ...
    if(*((u32*)&header[0x20]) <0x4000) gl_homebrew=true;
  //if(homebrew==false){
